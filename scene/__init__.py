    ...
    def __init__(self, args: ModelParams, gaussians: GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0], load_coarse=False):
        ...
        if os.path.exists(os.path.join(args.source_path, "sparse")) and args.extra_mark is None:
            ...
            scene_info = sceneLoadTypeCallbacks["endonerf"](
                args.source_path, mode=args.mode)
            ...
            scene_info = sceneLoadTypeCallbacks["scared"](
                args.source_path, mode=args.mode, init_pts=args.init_pts)
        ...
        if self.loaded_iter:
            iteration_str = 'iteration_' + \
                str(self.loaded_iter) if not load_coarse else 'coarse_iteration_' + \
